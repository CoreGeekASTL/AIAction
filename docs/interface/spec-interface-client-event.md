...
- **27.0 起注入终端白名单鉴权**：两个事件接口在写事件前调用 `AuthService.Check(imei,imsi)`，未命中白名单（或格式非法）返回 `code=401, msg:"auth rejected"`，白名单表为空（逃生态）时放行（src/controllers/event_controller.go；27.0 终端鉴权需求，2026-07-30）
