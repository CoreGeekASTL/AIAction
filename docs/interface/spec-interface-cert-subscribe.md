...
- **订阅失败即退出**：src/main.go:203-206（SubscribeCert 失败 `Fatalf`，进程直接退出，启动期依赖证书平台可用）
- **SDK 为本地桩**：src/stubs/CSPGSOMF/CertSDK/api/certapi/certapi.go:12（仓内 CertSDK 为 mock 实现，LOCAL_MODE 下订阅为空操作）
