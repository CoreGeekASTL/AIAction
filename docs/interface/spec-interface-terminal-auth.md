...
- **无接口级认证**：三接口仅内网监听，若监听地址暴露到非受信网络，白名单可被任意覆盖或泄露（27.0 终端鉴权需求裁定4）
- **逃生态**：白名单表为空时 AuthIMEI 全放行，导入首批白名单后自动失效（缓存被清空）
- **注入点联动**：登录三接口与事件两接口已注入同一鉴权服务，拒绝行为见 spec-interface-device-login.md / spec-interface-client-event.md（27.0 终端鉴权需求，2026-07-30）
