...
- **同组接口双实现**：src/controllers/exlogin_controller.go:42 与 src/controllers/login_controller.go:94（登录逻辑两份拷贝，改动易漏一边）
- **鉴权失败语义**：登录鉴权不通过时返回 `retcode.ClientFailed(-2)` + HTTP 400，而非 401（src/controllers/login_controller.go:152）
- **27.0 起注入终端白名单鉴权**：三条登录路径在建档前调用 `AuthService.Check(imei,imsi)`，未命中白名单（或格式非法）返回 `code=-2, msg:"auth rejected"`，白名单表为空（逃生态）时放行；内外两套 controller 均已注入（src/controllers/login_controller.go、src/controllers/exlogin_controller.go；27.0 终端鉴权需求，2026-07-30）
- **Muen 二次登录失败即整体失败**：src/controllers/login_controller.go:130-141（TikTok 场景 Muen login 或 UpdateUserToken 失败返回 InternalFailed + HTTP 400）
