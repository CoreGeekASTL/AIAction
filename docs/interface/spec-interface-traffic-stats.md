...
- **导出临时文件**：src/controllers/traffic_stats_controller.go:128-134（导出在本地临时目录生成 CSV+zip 后流式返回，依赖 `defer os.RemoveAll` 清理）
- **配套定时清理**：本功能域数据由定时任务按 `constants.CleanupMonths` 定期清理，见 [spec-interface-data-cleanup.md](spec-interface-data-cleanup.md)
