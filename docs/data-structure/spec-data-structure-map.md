...
> 类型：map　实例数：2　返回 [README.md](README.md)

## 1. 定位

map 在本仓承担"配置/指标索引"——监控模块用嵌套 map（外层 MocID→内层 set）建指标索引，并用 map 承载"指标 ID→采集函数"的函数注册表，是监控打点路由的数据中枢。
...
1. 新增嵌套 map（索引结构）须配 `sync.RWMutex` 保护，禁止裸 map 跨 goroutine 读写（依据：service/monitor_service.go:45-46）
2. 函数/策略注册表用 map 字面量在 init/start 阶段填充后运行期只读，禁止运行期动态改写（依据：service/monitor_service.go:146）
3. map 当 set 用时 value 统一 `struct{}` 而非 `bool`（依据：service/monitor_service.go:46/87，零内存语义）
