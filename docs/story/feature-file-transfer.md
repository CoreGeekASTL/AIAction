...
- 文件名入库前必须过 cleanFileName 防护（src/service/file_service.go）
- 大文件全量读内存，勿用于超限场景，插件包限 300MB（src/controllers/file_controller.go、src/common/constants/base.go）
- 内外两版 controller 同路径逻辑需同步修改（src/controllers/file_controller.go、src/controllers/exfile_controller.go）

## 7. 外部文档引用

| 文档类型 | 引用文档 | 引用点 |
|---|---|---|
| 关键类（必须） | [docs/key-class/README.md](../key-class/README.md) | BaseDao（File ORM 存取基座，src/dao/file.go 依赖）；本功能无专属关键类，文件服务为薄编排层（src/service/file_service.go） |
...
| 基础框架文档 | [rpc-beego-web.md](../framework-usage/rpc-beego-web.md) | Beego Web：路由注册与 multipart/文件流处理（src/routers/beego_router.go、src/controllers/file_controller.go） |
| 基础框架文档 | [storage-beego-orm.md](../framework-usage/storage-beego-orm.md) | Beego ORM：File 实体与自定义字节字段（src/dao/file.go、src/models/db/file.go） |
| struct 结构文档 | [spec-structure-AIAction.md](../structure/spec-structure-AIAction.md) | 本功能在 controllers/service/dao/models 分层中的位置 |
