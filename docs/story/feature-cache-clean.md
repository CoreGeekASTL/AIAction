...
- 出向调用沿用 5s 超时与仅认 200 约定（src/service/cache_service.go）
- 此功能是裸 http.Client 调用点，属禁模仿区，勿复制到新代码（src/service/cache_service.go）
- 失败审计字段 OperationZH/EN 成对填写（src/controllers/cache_controller.go）

## 7. 外部文档引用

| 文档类型 | 引用文档 | 引用点 |
|---|---|---|
| 关键类（必须） | [docs/key-class/README.md](../key-class/README.md) | BrowserService（取就绪实例，src/service/cache_service.go 调用 NewBrowserService）、Cse（实例注册表数据源，src/common/cse/cse.go） |
...
| 基础框架文档 | [rpc-go-chassis-cse.md](../framework-usage/rpc-go-chassis-cse.md) | CSE 服务发现：实例注册表（src/common/cse/cse.go） |
| struct 结构文档 | [spec-structure-AIAction.md](../structure/spec-structure-AIAction.md) | 本功能在 controllers/service/common 分层中的位置 |
