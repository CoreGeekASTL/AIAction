...
- IMEI 设 pk，IMSI 不加 pk，UNIQUE 联合索引走 DDL（需求 §7；src/dao/db_init.go）
- GaussDB 与 SQLite 双 DDL 必须同步（src/dao/db_init.go、src/dao/db_local_sqlite.go）
- CSV 按纯数据解析无 header，重复组合整批拒绝（裁定5）
- 登录拒绝用 ClientFailed(-2)，事件拒绝用 AuthFailed(401)（需求 §2.2.3；src/common/retcode）
- 新 Controller 只动 RouteInfo()，注册由路由层统一遍历（src/controllers/login_controller.go）

## 7. 外部文档引用

| 文档类型 | 引用文档 | 引用点 |
|---|---|---|
| 关键类（必须） | [docs/key-class/README.md](../key-class/README.md) | 复用 BaseController（请求解析与响应封装，src/controllers/controller.go）、BaseDao（ORM 基座，src/dao/base_dao.go）；在 LoginController（src/controllers/login_controller.go）与 EventService 入口（src/controllers/event_controller.go）注入鉴权调用 |
...
| 基础框架文档 | [rpc-beego-web.md](../framework-usage/rpc-beego-web.md) | Beego Web：新 Controller 按 RouteInfo 声明、注册进内部监听（src/routers/beego_router.go） |
| 基础框架文档 | [storage-beego-orm.md](../framework-usage/storage-beego-orm.md) | Beego ORM：实体三步曲+BaseInterface 继承+双 DDL（src/dao/base_dao.go） |
| 基础框架文档 | [di-singleton.md](../framework-usage/di-singleton.md) | 鉴权/管理 Service 按接口+小写实现+sync.Once 单例模式（src/service/user_service.go 模式） |
| 基础框架文档 | [concurrency-goroutine-sync.md](../framework-usage/concurrency-goroutine-sync.md) | 缓存组件 sync.RWMutex 读写锁与锁内惰性清理（需求 §2.2.4） |
| struct 结构文档 | [spec-structure-AIAction.md](../structure/spec-structure-AIAction.md) | 新模块在 controllers/service/dao/models 分层中的归属依据 |
