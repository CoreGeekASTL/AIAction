...
| 对外接口 | 新增 3 个内部接口：`POST /auth/v1/authIMEI`、`POST /auth/v1/importIMEIList`、`GET /auth/v1/exportIMEIList`，仅内部路由 | ✅ 已沉淀至 `docs/business/interface/spec-interface-terminal-auth.md` |
| 数据表 | 新增 `t_white_list`，GaussDB/SQLite 双 DDL 同步 | ✅ 双写一致 |
| 关键数据结构 | 新增 `authCache`（自定义带锁 map 缓存，容量 1000/TTL 30min/惰性清理 500） | ✅ 已沉淀至 `docs/business/data-structure/spec-data-structure-auth-cache.md` |
...
| 并发模型 | `authCache` 读写锁保护、无独立 goroutine、清理在写锁内完成 | ✅ 无 configCenter.configs 式无锁裸 map 隐患（见 `docs/business/data-structure/README.md` 全局风险） |

### 1.2 架构资产同步状态

| 资产 | 路径 | 状态 |
| --- | --- | --- |
| 接口文档 | `docs/business/interface/spec-interface-terminal-auth.md` | ✅ 已含 3 个新接口 |
| feature 文档 | `docs/business/story/feature-terminal-auth.md` | ✅ 已存在 |
| 数据结构文档 | `docs/business/data-structure/spec-data-structure-auth-cache.md` | ✅ 已含 authCache |
| 关键类清单 | `docs/business/key-class/README.md` | ❌ **缺口**：缺 `AuthController`、`AuthService`、`AuthManageService`、`WhiteListDao`、`authCache` 5 个新关键类，需补齐 |
| 包结构文档 | `docs/architecture/module-structure/` | 无新增包，无需变更 |
