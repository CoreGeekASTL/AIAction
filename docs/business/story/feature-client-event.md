...
| 文档类型 | 引用文档 | 引用点 |
|---|---|---|
| 关键类（必须） | [docs/business/key-class/README.md](../key-class/README.md) | EventService（事件上报统一入口，src/service/event_service.go） |
...
| 基础框架文档 | [rpc-beego-web.md](../../technical/framework-usage/rpc-beego-web.md) | Beego Web：路由注册与请求处理（src/routers/beego_router.go、src/controllers/event_controller.go） |
| 基础框架文档 | [di-singleton.md](../../technical/framework-usage/di-singleton.md) | 存储工厂 sync.Once 单例初始化（src/service/event_service.go） |
| struct 结构文档 | [spec-structure-AIAction.md](../../architecture/module-structure/spec-structure-AIAction.md) | 本功能在 controllers/service/common/models 分层中的位置 |
