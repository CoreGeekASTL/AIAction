...
flowchart LR
    subgraph 新增链路
        AC["AuthController<br/>(内部路由 9090)"] --> AS["AuthService<br/>联合鉴权+缓存"]
        AC --> AMS["AuthManageService<br/>CSV导入导出"]
        AS --> AC2["authCache<br/>带锁map+TTL+惰性清理"]
        AS --> WLD["WhiteListDao"]
        AMS --> WLD
        AMS -.导入成功后ClearCache.-> AS
        WLD --> WL[("t_white_list<br/>imei pk + imsi 联合唯一索引")]
    end
    subgraph 既有链路织入
        LC["LoginController.loginAuth"] -.前置拦截.-> AS
        ELC["ExLoginController.loginAuth"] -.前置拦截.-> AS
        EC["EventController.SendClientEvent/<br/>SendAppUseTimesEvent"] -.前置拦截.-> AS
    end
```
