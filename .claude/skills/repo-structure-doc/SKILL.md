...
输入一个代码仓路径或一个目录路径，输出一份《结构文档》。文档要回答两个问题：
...
| 输入 | 粒度 | 画什么 |
| --- | --- | --- |
| 代码仓根路径 | 仓库级 | 业务代码**第一层目录**之间的关系 |
| 某个子目录路径 | 目录级 | 该目录下**包与包**之间的关系 |

文档最终落到被分析代码仓根目录的 `docs/actor/` 下（不存在则创建）。

本 skill 通用，不预设任何特定代码仓。执行时基于实际探测结果走，禁止凭仓库名或已知项目结构臆测。

## 何时触发

- 用户给出代码仓路径或目录路径，要求"看下结构"、"梳理目录"、"画一下模块图"、"生成结构文档"。
...
只承载**业务代码第一层目录**之间的关系。辅助目录（构建产物、配置、文档、测试等）不画进图。理由：业务目录关系是结构文档的核心价值，把 `docs/`、`build/` 这类辅助目录画进来会冲淡主线、让图变成目录清单。
...
- 拿不准时：列出输入路径下一层内容，按"是否存在构建文件"二选一，并在文档概览的"业务目录识别依据"里注明判定理由。
...
4. 剩下的即业务代码第一层目录。若某目录名不在排除清单但明显是辅助目录（如 `vendor` `third_party` `mocks` `fixtures` `examples` `demo` `bench`），也排除，并在文档概览的"业务目录识别依据"里注明排除理由——这样可追溯、可质疑。
...
- 对每条依赖记录证据 `文件路径:行号`，供表格"主要依赖"列引用。

### 第 5 步：画 mermaid 图

用 `flowchart LR`（左到右，模块多时换 `TD`）。约定：
...
- 文档保存到**被分析代码仓根目录**的 `docs/actor/` 下（不是当前工作目录的 `docs/actor/`，除非两者相同）。
- 模式 A 文件名：`{仓库名}-结构文档.md`，仓库名取 git 仓库名或根目录名。
- 模式 B 文件名：`{目录名}-结构文档.md`。
- `docs/actor/` 不存在则创建（连同 `docs/` 一起创建）。
- 若同名文件已存在，覆盖前提示用户确认。
...
- 模块数：{N}
- 业务目录识别依据：{构建文件名 + 排除清单命中情况 + 额外排除项及理由}
- 依赖判定依据：import 语句扫描，证据见各模块"主要依赖"列
- 未识别项：{列出所有"未识别"的结论及原因；无则写"无"}
...
- **基于实证**：所有"依赖""职责"结论必须有 import 语句或文件路径支撑。读不到就写"未识别（原因：xxx）"，禁止凭目录名直接下依赖结论——`controllers` 不一定依赖 `services`，看代码说了算。
- **只读不改**：只读、只分析、只产出文档，不改动被分析代码仓的任何文件（新建 `docs/actor/` 目录和文档文件除外）。
- **语言无关**：不预设被分析仓的语言，按第 1 步实际探测结果走。
- **业务目录识别靠启发式不靠硬编码**：第 2 步的辅助目录清单是行业共识，可以排除；遇到清单外的可疑辅助目录，单独注明理由排除，保留可追溯性。
- **成品纯净**：最终文档只含成品内容（标题、概览、图、表格）。模板顶部的元说明、写作指令行、占位符说明均为规则，不复制进成品。第 1~4 步的探测过程（执行的 grep/rg 命令、命中输出摘要）仅供自检，绝不写入最终文档——其结论须以 `文件路径:行号` 证据形式进入相关表格。
