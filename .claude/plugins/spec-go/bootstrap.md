...
<EXTREMELY-IMPORTANT>
你拥有 spec-go。在执行任何代码仓分析、需求/设计文档解读、对外接口盘点、目录结构梳理、框架使用模式梳理、story 设计任务之前——即使你认为只有 1% 的可能某个 spec skill 适用——你也必须先用 Skill 工具加载该 skill 全文并遵循其指引。这不可协商、不可选择、不可用"我先看看代码"为自己开脱。
...
### 4. spec-framework-usage-analyze
...
### 5. spec-logic-audit
基于多彩建模方法论审核 Spec/需求文档中业务逻辑描述的完备性。当用户需要解读一篇 Spec/需求/设计文档、检查其中业务逻辑是否存在断裂点或描述不全、需要针对逻辑缺口 ask-human 补齐、并将建模结果以 HTML 可视化呈现供编码人员审核时使用。触发场景包括："审核 spec 业务逻辑完备性"、"多彩建模"、"检查需求逻辑断点"、"建模结果生成 HTML 并找人确认"等。
...
3. **接口归纳为功能域** → spec-feature-analyze：feature-*.md（L1 多彩建模 + L2 结构地图 + L3 AI 编码指南）
4. **框架使用模式** → spec-framework-usage-analyze：每框架一篇使用指导，归档 docs/framework-usage/
5. **需求文档逻辑审核** → spec-logic-audit：多彩建模 + HTML 可视化 + ask-human 补逻辑断点
6. **需求到 story 设计** → spec-story-design：产出与 docs/story/ 同构的新功能设计文档
